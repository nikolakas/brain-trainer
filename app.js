...
function _buildNodes() {
  ...
  for (const idx of _ALL_NODE_AAL) {
    ...
    const core = new THREE.Mesh(
      coreGeo,
      new THREE.MeshBasicMaterial({ color: baseHex, transparent: true, opacity: 0.95 }),
    );
    ...
    halo.position.copy(pos); halo.scale.setScalar(11);

    const node = { idx, core, halo, baseHex, pos };
    core.userData.node = node; halo.userData.node = node;   // for raycast picking
    _brainGroup.add(core); _brainGroup.add(halo);
    _nodes.push(node);
  }
}
...
function _refreshNodes() {
  for (const n of _nodes) {
    ...
    n.core.material.opacity = s.active ? 1.0 : (_tourMode ? 0.12 : 0.9);
    n.halo.material.color.setHex(hex);
    n.halo.material.opacity = s.glow * (s.active ? 0.95 : 0.55);
    n._targetScale = s.active ? 15 : 11;   // animated toward in the loop
    n._pulse = s.active;
  }
}
...
function _loop(now) {
  ...
  // node pulse + halo scale easing
  for (const n of _nodes) {
    const pulse = n._pulse ? 1 + 0.18 * Math.sin(t * 4 + n.idx) : 1;
    const target = (n._targetScale ?? 11) * pulse;
